...
> **Источник истины для всех репо.** Универсальную часть редактировать только здесь — синхронизировать через `scripts/sync_claude.sh`.
...
## Работа с вики — полуавтомат

Предлагать сохранить знание в вики **только** когда выполнены все три условия:
1. Концепт **новый** — не был в `wiki/INDEX.md` в начале сессии
2. Концепт **нетривиальный** — реальное открытие или решение, не очевидный вывод
3. К теме **возвращались** хотя бы дважды в сессии, или пользователь сказал «важно» / «запомни»

Формулировка предложения (одна строка, без давления):
> «Это стоит сохранить в вики — добавить в `raw/`?»

Если «да» — создать `raw/session_<YYYY-MM-DD>_<slug>.md` и запустить ingest. Если «нет» — не возвращаться.

**Не предлагать** если: концепт уже в INDEX, тема операционная (идёт в `lessons.md`), или предложение прозвучало в этой сессии 2+ раза.
...
2. Если репо определяет setup-script — выполнить (см. Локальные правила)
3. Прочитать `SYSTEM.md`, `CLAUDE.md`, `MEMORY/lessons/lessons.md`
4. Прочитать `MEMORY/tasks/todo.md` — только секции с меткой `[актуально]`, игнорировать `[устарело]`
5. Выполнить дополнительные шаги из Локальных правил если есть
6. Если в todo есть задача — спросить пользователя продолжить ли её или взять новую
